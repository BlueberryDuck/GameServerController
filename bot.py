...
class ServerManager:
    ...
    def start_log_monitoring_thread(self):
        ...
        """
        if self.log_monitoring_thread is None or not self.log_monitoring_thread.is_alive():
            self.log_monitoring_thread = threading.Thread(target=self.monitor_container_logs, daemon=True)
